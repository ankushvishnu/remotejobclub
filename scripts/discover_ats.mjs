...
async function fetchAndDiscover(url) {
  try {
    ...
    const res = await fetchWithTimeout(url, { timeout: 5000 }).catch(() => null);
...
function getCompanyName(url, token) {
  try {
    const host = new URL(url).hostname;
    let name = host.replace('www.', '').split('.')[0];
    if (['jobs', 'boards', 'apply', 'careers', 'grnh'].includes(name.toLowerCase())) {
        name = token;
    }
...
async function main() {
  ...
  if (filePaths.length === 0) {
    console.error("Usage: node scripts/discover_ats.mjs <file1.md> <file2.md> ...");
    process.exit(1);
  }
  ...
  for (const filePath of filePaths) {
    try {
      ...
      console.log(`\nScanning ${uniqueUrls.length} unique URLs from ${path.basename(filePath)}...`);
      ...
      console.log(`Discovered ${count} ATS targets in ${path.basename(filePath)}`);
    } catch (err) {
  ...
  if (uniqueResults.length > 0) {
    ...
    const filename = `${migrationsDir}/${timestamp}_seed_discovered_ats.sql`;
